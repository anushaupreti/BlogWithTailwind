...
<div class="w-4/5 m-auto pt-20">
  <form action="/blog/{{ $post->slug}}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <input type="text" name="title" placeholder="Title..." class="bg-transparent border-b-2 w-full h-20 text-6xl outline-none" value="{{ $post->title }}">

    <textarea name="description" id="" cols="30" rows="10" placeholder="Description..." class="py-20 bg-transparent  block border-b-2 w-full h-60 text-xl outline-none" >{{$post->description}}</textarea>
